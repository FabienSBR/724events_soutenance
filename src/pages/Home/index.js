...
// * Constante qui utilise des données provenant d'un contexte pour extraire et afficher des informations sur le dernier événement d'une liste *
const Page = () => {
  const { data } = useData();
  const last = data ? data.events[data.events.length - 1] : null; // Récupère le dernier élément du tableau ; si data est nul ou indéfini, assigne null à la variable last
  // data ? data.events[data.events.length - 1] à la place de const { last } = useData(); 
  // permet d'extraire et afficher une info spécifique : le dernier event en date, affiché dans la vignette du footer
 
  // Ajouts dans 'return' : 
  // sections "Nos services", "Nos réalisations" et "Notre équipe" associées chacune à un ID spécifique pour permettre un renvoi à la section concernée au clic dans la navbar
  return (
    <>
      <header>
        <Menu /> 
